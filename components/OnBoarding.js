    ...
    return (
        <Onboarding
            pages={onboardingData.map((item, index) => ({
                ...item,
                image: <Image source={item.image} style={{ width: '100%', height: '100%', resizeMode: 'contain' }} />,
                key: index.toString(),
            }))}
            titleStyles={{ fontSize: 40, fontWeight: 'bold', textAlign: 'center', color: 'white', marginTop: '-150px', marginLeft: 10, marginRight: 10 }}
            subTitleStyles={{ fontSize: 16, textAlign: 'center', color: 'white', marginLeft: 5, marginRight: 5 }}
